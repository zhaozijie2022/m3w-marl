...
from m3w.utils.envs_tools import get_shape_from_obs_space
from m3w.utils.envs_tools import check
from m3w.utils.models_tools import update_linear_schedule
from m3w.models.world_models import create_mlp
...
class WorldModelPolicy(nn.Module):
    def __init__(self, args: Dict, obs_space, action_space, device=torch.device("cpu")):
        super(WorldModelPolicy, self).__init__()
        ...
        self.act_limit = torch.tensor(action_space.high[0]).to(**self.tpdv)
        self.to(device)
...
class WorldModelActor:
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        assert act_space.__class__.__name__ == "Box", "WorldModel only supports continuous action space"

        self.tpdv = dict(dtype=torch.float32, device=device)
        self.lr = args["lr"]
        self.device = device

        self.actor = WorldModelPolicy(args, obs_space, act_space, device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr)
        self.turn_off_grad()

    def get_actions(self, obs, available_actions=None, stochastic=True):
        assert available_actions is None, "WorldModelActor only supports continuous action space"

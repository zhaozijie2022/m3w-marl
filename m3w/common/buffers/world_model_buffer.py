...
import numpy as np
import torch
from m3w.utils.envs_tools import get_shape_from_obs_space, get_shape_from_act_space
...
class WorldModelBuffer:
    ...
    def sample_horizon(self, horizon=None, t0=None):
        """
        sample length of horizon (n_agents, horizon, batch_size, *dim)
        """
        horizon = self.n_step if horizon is None else horizon

        self.update_end_flag()  # which transition is the last step of an episode
        if t0 is None:
            t0 = torch.randperm(self.cur_size).numpy()[:self.batch_size]  # initial timestep
        indices = [t0]
